...
function calc_next_node(current::Node, c_id::Int64,
    ...
    # 이동거리(arc length) ds에 대해 적분: Euler 수치적분
    ...
    if d > 0
        ...
        direction = false
        addedcost += abs(arc_l) * BACK_COST # 후진 페널티 5
    end

    # swich back penalty
    if direction != current.direction # switch back penalty
        addedcost += SB_COST # 전/후진 전환 페널티 100
    end
    ...
    # jacknif cost
    addedcost += JACKKNIF_COST * sum(abs.(rs_path.pi_2_pi.(yawlist-yaw1list))) # 잭나이프 페널티 200
